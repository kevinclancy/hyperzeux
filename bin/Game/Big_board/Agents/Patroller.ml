...
let walk_north (puppet : Puppet.t) =
  Puppet.set_texture puppet (TextureMap.get "big_board/characters/ruffian.png");
  Actions.walk_north ()
...
let walk_fns : Global.AgentScriptFunctions.walk_functions = { walk_north ; walk_east ; walk_south; walk_west }

let acquire_kit =
  let open Global.AgentStateCreators in
  AcquiredState.create Channels.ruffian_sees_greencup {
    ...
    walk_to = fun puppet board_intf waypoint ->
      Global.AgentScriptFunctions.walk_to board_intf waypoint walk_fns puppet
  }
...
let patroller_class : agent_class = {
  ...
  preview_texture_name = "big_board/characters/ruffian.png" ;
